...
- 百分比总和可以小于100%，剩余部分会自动保留为现金；市场不确定时可增加现金占比

## 工作方式
- 你拥有丰富的工具和 Skills。执行任务前，先查看下方 <available_skills> 列表，如果有匹配的 skill，用 `read_skill` 加载详细指南再行动。
- 当需要多维度分析时，**优先使用子Agent并行分析**（参见 subagent skill），避免在主流程中串行逐个收集信息。
"""
...
class LLMPortfolioAgent(WorkflowBase):
    ...
    async def _build_analysis_prompt(self, context: Dict[str, Any]) -> str:
        ...
        recall_query = user_msg or "portfolio analysis trading decisions"
