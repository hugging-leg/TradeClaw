...
- 交易日 / 市场开放 guard（在执行时检查，不使用 closure）
...
IMPORTANT: APScheduler + SQLAlchemyJobStore 使用 pickle 序列化 job func。
  - 只有**模块级函数**可以被正确 pickle/unpickle
  - closure / lambda / bound method 都**不能**被 pickle
  - 所有传给 add_job 的 func 必须是模块级函数
"""
...
from agent_trader.utils.time_utils import (
...
# ---------------------------------------------------------------------------
# Module-level APScheduler callback — 可被 pickle 序列化
# ---------------------------------------------------------------------------
# APScheduler + SQLAlchemyJobStore 用 pickle 序列化 job.func。
# pickle 要求 func 是模块级函数（通过 __module__ + __qualname__ 定位）。
# closure / lambda / bound method 都无法被正确 pickle。
#
# 解决方案：
#   1. 所有 job 统一使用这个模块级函数 _guarded_job_runner 作为回调
#   2. 实际要执行的函数以 "func_ref" (str) 形式存储在 job.kwargs 中
#   3. 交易日/市场开放 guard 参数也存储在 job.kwargs 中
#   4. 执行时动态 import + resolve 真正的函数并调用
# ---------------------------------------------------------------------------

async def _guarded_job_runner(**kwargs: Any) -> Any:
    """
    APScheduler 统一回调入口（模块级函数，可被 pickle）。

    kwargs 中包含：
      - _func_ref: str  — 真正要执行的函数引用，格式 "module.path:func_name"
      - _require_trading_day: bool
      - _require_market_open: bool
      - _exchange: str
      - _timezone_str: str
      - _job_id: str
      - 其余 kwargs 传给真正的函数
    """
    # 提取 guard 元数据
    func_ref: str = kwargs.pop("_func_ref")
    require_trading_day: bool = kwargs.pop("_require_trading_day", False)
    require_market_open: bool = kwargs.pop("_require_market_open", False)
    exchange: str = kwargs.pop("_exchange", "XNYS")
    timezone_str: str = kwargs.pop("_timezone_str", "US/Eastern")
    job_id: str = kwargs.pop("_job_id", "")
    ...
    # Resolve and call the actual function
    func = _resolve_func_ref(func_ref)
    return await func(**kwargs)


def _resolve_func_ref(func_ref: str) -> Callable[..., Coroutine]:
    """
    将 "module.path:func_name" 解析为实际的函数对象。

    例如: "agent_trader.trading_system:_llm_scheduled_trigger"
    """
...
def make_func_ref(func: Callable) -> str:
    """
    将模块级函数转换为 "module.path:func_name" 字符串引用。

    仅接受模块级函数（非 closure / lambda / bound method）。
    """
    module = func.__module__
    qualname = func.__qualname__
    # 模块级函数的 __qualname__ 等于 __name__（不含 "." 或 "<"）
    if "." in qualname or "<" in qualname:
        raise ValueError(
            f"只能引用模块级函数，但 {func!r} 的 qualname={qualname!r} "
            f"表明它是 closure / lambda / 嵌套函数。"
            f"请将其提取为模块级函数。"
        )
...
class SchedulerMixin:
    ...
    def _add_job(
        ...
        *,
        ...
        将 func 转换为 "module:name" 字符串引用，连同 guard 参数一起
        存入 job kwargs，使用模块级 _guarded_job_runner 作为实际回调。
        这样 APScheduler 的 pickle 序列化可以正常工作。
        """
        try:
            func_ref = make_func_ref(func)
        except ValueError as e:
            logger.error("添加任务失败 %s: %s", job_id, e)
            return False

        # 合并用户 kwargs 与 guard 元数据
        merged_kwargs = dict(kwargs or {})
        merged_kwargs.update({
            "_func_ref": func_ref,
            "_require_trading_day": require_trading_day,
            "_require_market_open": require_market_open,
            "_exchange": self._exchange,
            "_timezone_str": settings.trading_timezone,
            "_job_id": job_id,
        })
    ...
    async def get_execution_history(
        ...
        from agent_trader.db.models import SchedulerJobExecution
    ...
    def _adjust_to_trading_day(self, dt: datetime) -> datetime:
        ...
        try:
            ...
            adjusted = dt.replace(year=next_day.year, month=next_day.month, day=next_day.day)
            logger.info("调整到下一个交易日: %s -> %s", dt.isoformat(), adjusted.isoformat())
            return adjusted
        except Exception as e:
