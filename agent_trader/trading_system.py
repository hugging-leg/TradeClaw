...
from agent_trader.agents import WorkflowFactory, discover_external_workflows
...
from agent_trader.memory import MemoryManager
...
from agent_trader.utils.time_utils import parse_time_config
from agent_trader.utils.timezone import utc_now


# ---------------------------------------------------------------------------
# Module-level trampoline functions for APScheduler
# ---------------------------------------------------------------------------
# APScheduler + SQLAlchemyJobStore 使用 pickle 序列化 job func。
# bound method (self.xxx) 经过 pickle 反序列化后会丢失 self 绑定。
# 所有 APScheduler 回调必须是模块级函数，运行时动态获取 TradingSystem 实例。
# ---------------------------------------------------------------------------


async def _llm_scheduled_trigger(**kwargs: Any) -> None:
    """APScheduler 回调 trampoline：LLM 自调度触发 workflow"""
    from agent_trader.api.deps import get_trading_system
    ts = get_trading_system()
    await ts.trigger_workflow(**kwargs)


async def _scheduled_trigger_workflow_trampoline(
    trigger: str = "unknown", **extra_context: Any,
) -> None:
    """APScheduler 回调 trampoline：定时触发 workflow（如 daily_rebalance）"""
    from agent_trader.api.deps import get_trading_system
    ts = get_trading_system()
    await ts.trigger_workflow(trigger=trigger, context=extra_context or None, fire_and_forget=False)


async def _scheduled_eod_analysis_trampoline() -> None:
    """APScheduler 回调 trampoline：收盘分析"""
    from agent_trader.api.deps import get_trading_system
    ts = get_trading_system()
    await ts._run_eod_analysis()


async def _scheduled_portfolio_check_trampoline() -> None:
    """APScheduler 回调 trampoline：组合检查"""
    from agent_trader.api.deps import get_trading_system
    ts = get_trading_system()
    await ts._scheduled_portfolio_check()


async def _scheduled_risk_check_trampoline() -> None:
    """APScheduler 回调 trampoline：风控检查"""
    from agent_trader.api.deps import get_trading_system
    ts = get_trading_system()
...
class TradingSystem(SchedulerMixin):
    ...
    def __init__(self) -> None:
        ...
        # ---- 外部 Workflow 插件 ----
        external_agents_dir = str(Path(settings.data_dir) / "agents")
        discover_external_workflows(external_agents_dir)

        # ---- Memory Manager ----
        self.memory_manager = MemoryManager(postgres_uri=settings.postgres_uri)

        # ---- Workflow ----
        # workflow 在 start() 中创建（需要先 await memory_manager.initialize()）
    ...
    async def start(self, enable_trading: bool = True) -> bool:
        ...
        try:
            ...
            # Realtime Monitor
            if self.trading_workflow.supports_realtime_monitoring:
    ...
    # ------------------------------------------------------------------
    # Memory-aware Workflow 创建
    # ------------------------------------------------------------------

    def _rebuild_workflow_with_memory(
        self, workflow_type: Optional[str] = None
    ) -> None:
        """
        使用 MemoryManager 提供的 checkpointer + store 创建/重建 workflow。

        在以下场景调用：
        - start() 中 memory_manager 初始化完成后
        - switch_workflow() 切换 workflow 类型时
        """
        wf_type = workflow_type or settings.workflow_type
        ...
        # 注入 TradingSystem 引用（供 LLM self-scheduling 使用）
        self.trading_workflow._trading_system = self
        logger.info("Workflow %s created with persistent memory", wf_type)
    ...
    async def switch_workflow(self, workflow_type: str) -> Dict[str, Any]:
        ...
        async with self._workflow_lock:
            ...
            # 创建新 workflow（带持久化 memory）
            self._rebuild_workflow_with_memory(workflow_type)
    ...
    async def send_chat_message(self, message: str) -> Dict[str, str]:
        """
        向 Agent 发送用户消息。

        - 如果 workflow 正在运行：消息追加到队列，Agent 会在当前 ReAct 循环结束后处理
        - 如果 workflow 空闲：触发新的 workflow 执行，以用户消息作为 prompt

        Returns:
            {"status": "queued" | "triggered", "message": str}
        """
        if self.trading_workflow.is_running:
            self.trading_workflow.enqueue_user_message(message)
            queue_size = self.trading_workflow.chat_queue_size
            return {
                "status": "queued",
                "message": f"Message queued (position: {queue_size})",
                "queue_size": queue_size,
            }
        else:
            # 空闲 → 触发新 execution，user_message 作为 context
            await self.trigger_workflow(
                trigger="user_chat",
                context={"source": "chat", "user_message": message},
            )
            return {
                "status": "triggered",
                "message": "New workflow execution started",
                "queue_size": 0,
            }
    ...
    async def _execute_workflow_once(
        ...
        try:
            ...
            # 调用 execute()（模板方法）：自动处理 DB 持久化、事件广播
            result = await self.trading_workflow.execute(ctx)
    ...
    def _register_scheduled_jobs(self) -> None:
        ...
        IMPORTANT: 所有 func 必须是模块级函数（不能是 self.xxx bound method），
        因为 APScheduler + SQLAlchemyJobStore 使用 pickle 序列化 job func。
        """
        ...
        self.add_cron_job(
            ...
            func=_scheduled_trigger_workflow_trampoline,
            hour=rebalance_h,
            minute=rebalance_m,
        ...
        self.add_cron_job(
            job_id="eod_analysis",
            func=_scheduled_eod_analysis_trampoline,
            hour=eod_h,
            minute=eod_m,
            require_trading_day=True,
        )
        ...
        self.add_interval_job(
            ...
            func=_scheduled_portfolio_check_trampoline,
        ...
        if self.risk_enabled:
            self.add_interval_job(
                job_id="risk_check",
                func=_scheduled_risk_check_trampoline,
