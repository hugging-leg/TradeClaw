...
class TradingCommands(CommandHandler):
    ...
    async def handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            await self.publish_event("enable_trading", {"chat_id": ctx.chat_id})
            # 注意：实际的响应消息由 TradingSystem 通过事件处理后发送
    ...
    async def handle_stop(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            await self.publish_event("disable_trading", {
                "chat_id": ctx.chat_id,
                "reason": "Telegram 手动暂停"
            })
    ...
    async def handle_emergency(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await self.publish_event("emergency_stop", {"chat_id": ctx.chat_id})
